...
const ConnectToDB = async () => {
   try {
      ...
      changestream.on("change", async (change) => {
         ...
         notify_que.push({ id: 1, message:`auction deleted ${change.documentKey._id}`, date:moment().format('MMMM Do YYYY, h:mm:ss a') })
       ...
       if(change.operationType==='insert'){
         notify_que.push({ id: 1, message:`auction added ${change.fullDocument.name}`, date:moment().format('MMMM Do YYYY, h:mm:ss a') })
